...
const MyWork = () => {
  ...
  const [logModal, setLogModal] = useState(null); // assignment object
  ...
  const submitLog = async () => {
    if (!logUnits || logUnits <= 0) {
      setLogError("Enter units > 0");
      return;
    }
    if (Number(logUnits) > logModal.units_pending) {
      setLogError(`Max ${logModal.units_pending} units pending`);
      return;
    }
    setSaving(true);
    try {
    ...
    <div style={S.page}>
      ...
      {!loading && assignments.length > 0 && (
        <div style={S.summaryStrip}>
          ...
          <StatChip label="Completed"      value={totalCompleted} color="#2ecc71" />
          ...
          <div key={projectName} style={S.projectBlock}>
            <div style={S.projectHeader}>
              ...
              <div style={S.projectMeta}>
                <span style={S.metaChip}>{pCompleted}/{pAssigned} units</span>
            ...
            <table style={S.table}>
              ...
              <tbody>
                {rows.map((a) => {
                  const pct = a.units_assigned > 0
                    ? Math.round((a.units_completed / a.units_assigned) * 100) : 0;
                  const done = Number(a.units_pending) === 0;

                  return (
                    <tr key={a.assignment_id} style={done ? S.trDone : {}}>
                      ...
                      <td style={{ ...S.td, textAlign: "center", color: "#2ecc71", fontWeight: "700" }}>{a.units_completed}</td>
                      <td style={{ ...S.td, textAlign: "center", color: Number(a.units_pending) > 0 ? "#e74c3c" : "#2ecc71", fontWeight: "700" }}>
                        {a.units_pending}
                      </td>
      ...
      {logModal && (
        <div style={S.overlay}>
          <div style={S.modal}>
            <h3 style={S.modalTitle}>Log Progress</h3>
            <p style={S.modalSub}>
              <strong>{logModal.project_name}</strong> · {logModal.role} · {logModal.task_name}
            </p>
            <div style={S.modalInfo}>
              <span>Pending: <strong style={{ color: "#e74c3c" }}>{logModal.units_pending}</strong> units</span>
            </div>
            ...
            <div style={S.modalField}>
              ...
              <input type="number" value={logUnits} min="1" max={logModal.units_pending}
                onChange={(e) => { setLogUnits(e.target.value); setLogError(""); }}
                style={S.input} placeholder={`Max ${logModal.units_pending}`} />
            </div>
            <div style={S.modalField}>
  ...
  return (
    <div style={{ background: "#f0f0f0", borderRadius: "4px", height: "8px", overflow: "hidden" }}>
      <div style={{
        ...
        backgroundColor: color, transition: "width 0.5s ease", borderRadius: "4px"
      }} />
    </div>
  );
};
...
const S = {
  ...
  modal:       { background: "white", borderRadius: "12px", padding: "28px", width: "100%", maxWidth: "420px", boxShadow: "0 20px 60px rgba(0,0,0,0.3)" },
  modalTitle:  { fontSize: "18px", fontWeight: "800", color: "#1e272e", marginBottom: "6px" },
  modalSub:    { fontSize: "13px", color: "#777", marginBottom: "12px" },
  modalInfo:   { background: "#fff8f8", border: "1px solid #fde8e8", borderRadius: "6px", padding: "10px 14px", marginBottom: "16px", fontSize: "13px", color: "#555" },
  ...
  saveBtn:     { padding: "8px 22px", background: "#e74c3c", color: "white", border: "none", borderRadius: "6px", cursor: "pointer", fontWeight: "700" },
};
